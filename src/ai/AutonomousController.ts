...
import { AutonomousPlanner, AutonomousDecision, AutonomousContext } from './AutonomousPlanner.js';
...
export class AutonomousController {
  ...
  async executeAutonomousGoal(
    ...
    const steps: Array<{ action: string; success: boolean; data?: any; error?: string; }> = [];
    ...
    try {
      ...
      this.logger.error('Autonomous execution failed', error as Error, { goal });
  ...
  ): Promise<{ action: string; success: boolean; data?: any; error?: string; }> {
    try {
      ...
      this.logger.error('Decision execution failed', error as Error, {
        action: decision.action,
  ...
  private async executeScrapeAction(
    page: Page,
    decision: AutonomousDecision,
    context: AutonomousContext
  ): Promise<{ action: string; success: boolean; data?: any; error?: string; }> {
  ...
  private async executeFillAction(
    page: Page,
    decision: AutonomousDecision
  ): Promise<{ action: string; success: boolean; data?: any; error?: string; }> {
    const selector = decision.parameters.selector;
  ...
  private isGoalAchieved(goal: string, currentData: Record<string, any>, decision: AutonomousDecision): boolean {
    // Check if we have meaningful data related to the goal
    const hasData = Object.keys(currentData).length > 0;
    const hasRelevantData = Object.values(currentData).some(item => 
      item && typeof item === 'object' && item.text && item.text.length > 0
    );
  ...
  private calculateOverallConfidence(steps: Array<{ action: string; success: boolean; data?: any; }>): number {
  ...
  private assessDataQuality(steps: Array<{ action: string; success: boolean; data?: any; }>): number {
    const scrapeSteps = steps.filter(s => s.action === 'scrape' && s.success && s.data);
    if (scrapeSteps.length === 0) return 0;
      ...
      if (step.data) {
        for (const item of Object.values(step.data)) {
          if (item && typeof item === 'object' && 'text' in item) {
            const textLength = (item as any).text?.length || 0;
            const hasRelevantContent = textLength > 10 && textLength < 1000;
            const hasConfidence = (item as any).confidence || 0;
  ...
  private analyzePageStructure(html: string, url: string): {
    title: string;
    headings: string[];
    links: number;
    forms: number;
  ...
  private calculateExtractionConfidence(text: string | null, target: string, context: AutonomousContext): number {
    if (!text) return 0;
  ...
  private validateExtractionQuality(data: any, target: string, context: AutonomousContext): boolean {
    ...
    if (text.length > 2000) return false; // Probably extracted too much
    
    // Check for common extraction errors
    const commonErrors = ['null', 'undefined', 'NaN', '[]', '{}', 'function', 'object Object'];
    if (commonErrors.some(error => text.includes(error))) return false;
