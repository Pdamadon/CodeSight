...
export class TestRunner {
  ...
  private async runValidationTests(): Promise<TestSuite> {
    ...
    suite.tests.push(await this.runTest('Valid URL validation', () => {
      ...
      if (!result.success) throw new Error('Valid URL should pass validation');
    }));

    suite.tests.push(await this.runTest('Invalid URL validation', () => {
      const result = ValidationSystem.validateUrl('invalid-url');
      if (result.success) throw new Error('Invalid URL should fail validation');
    }));
  ...
  private async runDOMAnalyzerTests(): Promise<TestSuite> {
    const suite: TestSuite = {
      name: 'DOM Analyzer',
      tests: [],
      passed: 0,
  ...
  private async runLearningSystemTests(): Promise<TestSuite> {
    const suite: TestSuite = {
