...
export class InteractionPlanner {
  ...
  async planInteractions(request: InteractionRequest): Promise<InteractionPlan> {
  ...
  async executeInteractionPlan(page: Page, plan: InteractionPlan): Promise<{
    success: boolean;
    ...
    try {
      ...
      await this.learningSystem.recordInteractionPattern(
        page.url(),
  ...
  private async executeStep(page: Page, step: InteractionStep): Promise<void> {
    switch (step.type) {
      case 'click':
      ...
      case 'navigate':
        if (!step.value) throw new Error('Navigate step requires URL');
  ...
  private async extractFinalData(page: Page): Promise<Record<string, any>> {
  ...
  private buildInteractionPrompt(request: InteractionRequest): string {
    return `
...
`;
  ...
  private parseInteractionResponse(reasoning: string, goal: string): InteractionPlan {
  ...
  private generateFallbackPlan(goal: string): InteractionPlan {
    return {
      steps: [
        {
          type: 'wait',
